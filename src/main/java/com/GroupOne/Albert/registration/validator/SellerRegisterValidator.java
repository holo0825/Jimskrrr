...
public class SellerRegisterValidator implements Validator {
	private static final Pattern EMAIL_REGEX = Pattern.compile("^[\\w\\d._-]+@[\\w\\d.-]+\\.[\\w\\d]{2,6}$");	
	private static Logger log = LoggerFactory.getLogger(SellerRegisterValidator.class);
	@Override
...
//		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "tel", 		"", "電話欄不能空白");
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "companyName", 		"", "公司名稱欄不能空白");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "companyAddress", 		"", "公司地址欄不能空白");
